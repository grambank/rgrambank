...
skip("Skipping -- broken until fixed")
test_that("raw binarised features are being overwritten when keep_raw_binary is set to FALSE", {
   outcome <- binarise(
       ValueTable = readr::read_csv("fixtures/values_with_raw_fake_binary.csv", show_col_types = FALSE), wide = TRUE,
...
test_that("raw binarised features are not being overwritten when keep_raw_binary is set to TRUE", {
   outcome <- binarise(
       ValueTable = readr::read_csv("fixtures/values_with_raw_fake_binary.csv", show_col_types = FALSE),
       wide = TRUE,
       drop_multistate = FALSE,
