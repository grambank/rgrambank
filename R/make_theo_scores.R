...
ValueTable_ParameterTable <- ValueTable %>%
...
#fusion counts
df_morph_count <- ValueTable_ParameterTable %>%
  ...
  #  mutate(Value_weighted = ifelse(Fusion == 0, abs(Value-1), Value_weighted)) %>% # reversing the Values of the features that refer to free-standing markers
    dplyr::group_by(Language_ID) %>%
  dplyr::summarise(mean_morph = mean(Value_weighted))


##Flexivity scores
lg_df_for_flex_count <- ValueTable_ParameterTable  %>%
...
##`locus of marking`s
lg_df_for_HM_DM_count <- ValueTable_ParameterTable %>%
...
lg_df_for_gender_nc_count <- ValueTable_ParameterTable  %>%
...
##OV_VO scores
lg_df_for_OV_VO_count <- ValueTable_ParameterTable  %>%
...
lg_df_informativity_score <-  ValueTable_ParameterTable  %>%
