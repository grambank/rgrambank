...
#' @param drop_question_marks boolean. Remove values assigned to uncertain ("?")
...
language_level_df <- function(ValueTable, LanguageTable,
    ...
    if (method == "singular_least_missing_data") {
        levelled_ValueTable <- ValueTable %>%
            ...
            dplyr::ungroup() %>%
            dplyr::distinct("Language_level_ID", .keep_all = TRUE) %>%
    ...
    # MERGE BY MAKING A FRANKENSTEIN COMBINATION OF ALL THE DIALECTS
        ...
        ValueTable_grouped <- ValueTable %>%
            ...
            dplyr::left_join(LanguageTable, by = "Language_ID", relationship = "many-to-many") %>%
            dplyr::group_by("Language_level_ID", "Parameter_ID") %>%
            dplyr::mutate(n = dplyr::n()) %>%
            dplyr::ungroup() %>%
            dplyr::select(-"Language_ID")

        # it's faster if we do slice_sample (choose randomly) only on those that 
        # have more than 1 value per language than if we do it on all.
