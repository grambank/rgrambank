...
#' @param LanguageTable a filepath to a csv-sheet of a table with glottocodes and language-level glottocodes of the type LanguageTable in the cldf-ontology. if nothing is specified, it fetches table from glottolog-cldf online on GitHub (v4.8).
...
ValueTable_long_n_greater_than_1 <- ValueTable_long %>%
  filter(n > 1) %>%
  group_by(Language_level_ID, Parameter_ID) %>%
  slice_sample(n = 1) %>%
  ungroup()

levelled_ValueTable <- ValueTable_long %>%
  filter(n == 1) %>%
  full_join(ValueTable_long_n_greater_than_1 , by = c("Parameter_ID", "Value", "Language_level_ID", "n"))  %>%
  mutate(Language_ID = Language_level_ID) %>%
